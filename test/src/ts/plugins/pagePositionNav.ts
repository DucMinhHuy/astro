    ...
    attachEvent(controller: any) {
      ...
      for (let section of sections) {
        ...
        scene_pagePositionNav.on("enter", () => {
          ...
          // console.log(String(targetElement.getAttribute("data-page-section")))
          this.removeNavActiveClass();
          let el = document.querySelector(`[data-page-nav="${targetElement.getAttribute("data-page-section")}"]`);
          if (!el) {  
            throw new Error('buttonがありません！');  
          }
          el.classList.add(this.settings.addClassName);
        });
        ...
        //リサイズやスクロールでアップデート
        scene_pagePositionNav.on("update", function(this: any) {
